    ...
    m_buildItemMultiplierEnabled(false)
{
    ...
    d2tm_assert(buildingListUpdater!=nullptr);
    removeAllItems();
...
void cItemBuilder::itemIsDoneBuildingLogic(cBuildingListItem *item)
{
    ...
    if (eBuildType == STRUCTURE) {
        ...
        if (eBuildType == UNIT) {
            ...
            d2tm_assert(item->getTimesToBuild() > -1);
            ...
            if (special.providesType == UNIT) {
                ...
                if (special.deployFrom == AT_STRUCTURE) {
                    ...
                    // unsupported case
                    d2tm_assert(false && "Unsupported special case");
                }
...
{
    d2tm_assert(position > -1);
    d2tm_assert(position < MAX_ICONS);
...
{
    d2tm_assert(position > -1);
    d2tm_assert(position < MAX_ITEMS);
